...
<div class="page-header">
    ...
    <p>Update your studio booking request</p>
</div>
            ...
            <form method="POST" action="{{ route('bookings.update', $booking->booking_id) }}">
                    ...
                    <select name="studio_id" id="studio_id" class="form-control" style="font-size:13px;" required>
                        ...
                        @foreach($studios as $studio)
                            <option value="{{ $studio->studio_id }}"
                                {{ old('studio_id', $booking->studio_id) == $studio->studio_id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="date"
                           ...
                           min="{{ date('Y-m-d') }}"
                           value="{{ old('booking_date', \Illuminate\Support\Carbon::parse($booking->booking_date)->format('Y-m-d')) }}"
                           required>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-6">
                        ...
                        <input type="time"
                               ...
                               min="08:00"
                               max="23:30"
                               value="{{ old('start_time', \Illuminate\Support\Carbon::parse($booking->start_time)->format('H:i')) }}"
                               required>
                    </div>
                    <div class="col-6">
                        ...
                        <input type="time"
                               ...
                               min="08:00"
                               max="23:30"
                               value="{{ old('end_time', \Illuminate\Support\Carbon::parse($booking->end_time)->format('H:i')) }}"
                               required>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="purpose"
                              ...
                              style="font-size:13px;"
                              rows="3"
                              placeholder="e.g. Band practice, recording session">{{ old('purpose', $booking->purpose) }}</textarea>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadAvailability() {
        ...
        fetch(`${availabilityUrlBase}/${studioId}/availability?date=${date}&exclude=${currentBookingId}`, {
